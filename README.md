...
7. **Monetize & benchmark.** QALYs × VSLY (HHS 2026 central $611k) gives a
   ...
   (loguniform $50–150 **per DALY averted**, central ~$87) — converted to QALYs
   via `daly_to_qaly_factor` (1.0; an explicit assumption, since a DALY averted
   and a QALY gained are both ≈ one year of full health but not identical
   constructs) and *handicapped with the same realization and credibility* so it
   is a like-for-like comparison, not a raw ceiling — give the counterfactual
   benchmark.
...
<!-- RESULTS:START -->
**Median ≈ 106k QALYs** (mean 114k; 90% interval 60k–191k), a blended **$248k/QALY**. Monetized at VSLY that is **$64.6B** of health value — a **2.5× benefit/cost ratio**. The same $26.3B at the global-health frontier (~$87 per DALY averted, treated as ≈1 QALY), handicapped with the same realization and evidence discounts, would still buy ~207.91M QALYs — about **1955× more health per dollar**, the price of funding a rich country's social fabric rather than the global frontier.
