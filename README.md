...
3. **Dollar allocation.** The split across 13 intervention archetypes is a
   **Dirichlet** whose centers are **derived from Scott's own gift-level
   ...
   a sync test fails if `parameters.yaml` drifts from the derivation. The
   author-set concentration controls how tightly draws hold to those centers
   (undisclosed dollars are assumed to allocate like disclosed ones).
...
<!-- RESULTS:START -->
**Median ≈ 71k QALYs** (mean 77k; 90% interval 38k–139k), a blended **$430k/QALY**. Monetized at HHS's value per QALY that is **$52.3B** of health value — a **1.7× benefit/cost ratio**. The same $30.3B at the global-health frontier (~$197/QALY-equivalent), handicapped with the same realization and evidence discounts, would still buy ~105.51M QALYs — about **1490× more health per dollar**, the price of funding a rich country's social fabric rather than the global frontier.
...
- **Causal skepticism cuts the headline by roughly 65%.** Weighting each
  effect by its study design takes the central estimate from ~201k QALYs
  (trust every cited effect at face value) to ~71k. The buckets that survive are
  the ones with the strongest designs
  (collaborative-care RCTs, the Medicaid difference-in-differences); the largest
  *dollar* buckets (education ~20%, equity & justice ~16%, workforce ~14% per
...
- Allocation centers are derived from the two-thirds of dollars with disclosed
  amounts, assuming undisclosed gifts allocate the same way; a gift's dollars
  are split equally across its organization's focus areas (tags are org-level,
  not per-gift); and the area-to-archetype mapping embeds judgment (documented
  line-by-line in [`leaf_to_archetype.yaml`](data/yieldgiving/leaf_to_archetype.yaml)).
